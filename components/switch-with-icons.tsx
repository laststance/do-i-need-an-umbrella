...
export function SwitchWithIcons() {
  ...
  return (
    <button
      onClick={toggleTheme}
      className={`w-[56px] h-[28px] rounded-full p-1 relative transition-colors duration-500 ${
    ...
    >
      ...
      >
        {resolvedTheme === "dark" ? (
          <Moon className={`h-4 w-4 text-white transition-all duration-300 ${isAnimating ? "rotate-[360deg]" : ""}`} />
        ) : (
          <Sun className={`h-4 w-4 text-white transition-all duration-300 ${isAnimating ? "rotate-[360deg]" : ""}`} />
        )}
      </span>
    </button>
  )
